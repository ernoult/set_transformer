    ...
    if X.size(1) == 2:
        ...
        if labels is None:
            for i in range(X.size(0)):
                ax.add_patch(
                            Rectangle((X[i, 0].cpu().data.numpy(), X[i, 1].cpu().data.numpy()), 
                              X[i, 2].cpu().data.numpy(), X[i, 3].cpu().data.numpy(), 
            ...
            for (l, c) in zip(ulabels, colors):
                X_temp=X[torch.where(labels==l)[0]]

                #Put sample rectangles
                for i in range(X_temp.size(0)):
                    ax.add_patch(
                        Rectangle((X_temp[i, 0].cpu().data.numpy(), X_temp[i, 1].cpu().data.numpy()), 
                                  X_temp[i, 2].cpu().data.numpy(), X_temp[i, 3].cpu().data.numpy(), 
...
def rect_for_plots(rects):
    ...
    w = torch.abs(rects[:,2] - rects[:,0]).unsqueeze(1)
    h = torch.abs(rects[:,3] - rects[:,1])
    x2 = (rects[:,1] - h).unsqueeze(1)
    h = h.unsqueeze(1)
    x1 = rects[:, 0].unsqueeze(1)
    
    return torch.hstack((x1, x2, w, h))
...
def scatter_mog(X, labels, mu, cov, ax=None, colors=None):
    ...
    if X.size(1)==2:
        for i, l in enumerate(ulabels):
            draw_ellipse(mu[l].cpu().data.numpy(), cov[l].cpu().data.numpy(), ax=ax, fc=colors[i])
        ...
        for (l, c) in zip(ulabels, colors):
            ...
            ax.add_patch(
                Rectangle((mu_plot[l, 0].cpu().data.numpy(), mu_plot[l, 1].cpu().data.numpy()), 
                            ...
                            fill=False, color=c, linestyle ='-', linewidth=1.5, alpha=0.6
                            )
            )

            #Add ellipses for upper left and  bottom right corners
            draw_ellipse(mu[l,0:2].cpu().numpy(), cov[l, 0:2, 0:2].cpu().numpy(), ax=ax, fc=c)
            draw_ellipse(mu[l,2:4].cpu().numpy(), cov[l, 2:4, 2:4].cpu().numpy(), ax=ax, fc=c)
